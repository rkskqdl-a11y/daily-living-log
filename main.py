...
def generate_content(post_type, keyword, product=None):
    ...
    try:
        ...
        # [수정] 광고글일 때 AI가 누락 없이 작성하도록 프롬프트 강화
        if post_type == "AD":
            prompt = f"""
            건강 전문 에디터로서 '{keyword}'의 효능을 설명하고 아래 제품을 추천하는 HTML 글을 1,500자 이상 작성하세요.
            [추천 제품]: {product['productName']}
            [구매 링크]: {product['productUrl']}
            
            구성 규칙:
            1. 도입부 스타일: {ins}
            2. 본론 스타일: {bs} (반드시 <table>을 포함하고 <div class='table-container'>로 감싸세요)
            3. 결론 스타일: {os}
            4. 마크다운 기호(**, ##)는 절대 쓰지 마세요.
            5. 본문 중간에 <a href='{product['productUrl']}' style='font-weight:bold; color:blue;'>▶ 제품 상세정보 및 최저가 확인하기</a> 링크를 반드시 2회 이상 넣으세요.
            """
            # 이미지와 대가성 문구를 프로그래밍 방식으로 확실히 결합
            ...
            prompt = f"전문 에디터로서 '{keyword}'에 대해 1,500자 이상 HTML로 작성하세요. 구성:[도입-{ins},본론-{bs},결론-{os}]. <table>은 <div class='table-container'>로 감싸세요. 마크다운 기호는 쓰지 마세요."
            res = model.generate_content(prompt).text
...
def main():
    ...
    print(f"📢 {strategy['desc']} 가동 중 - [{post_type}] 발행 프로세스 시작: {kw}")
    
    if post_type == "AD":
        products = fetch_product(kw.split()[0])
        if products:
            ts, html = generate_content("AD", kw, products[0])
            # [수정] 광고글 제목 앞에 [강력추천] 태그를 붙여 확실히 구분
            ad_title = f"[강력추천] {ts} {kw} 건강 관리 가이드"
            if html and (url := post_to_blog(ad_title, html)):
                print(f"✅ 광고글 발행 성공: {url}")
        else:
            print("⚠️ 상품 검색 실패로 정보글로 자동 전환합니다.")
            post_type = "INFO"
